...
<span class="m">GET</span> <span class="u">https://api.verkada.com</span>
     ...
     <span class="c">header:</span> x-verkada-auth: &lt;トークン&gt;</pre>
      ...
      <div class="panel">
        ...
        <table class="params">
          ...
          <tr><td><code>camera_id</code></td><td class="d">対象カメラ（入口ごと・2拠点計5台）</td></tr>
    ...
    <div class="note">
      <b>ご確認事項（計測仕様）</b>
      <ul>
        <li>入退店は Verkada Command で各カメラに設定した「混雑傾向ライン」に基づく実測カウントです（2026-07-21 設定）。</li>
...
function boot() {
  ...
  async function loadWeather(dateStr) {
    try {
      const w = await (await fetch(`/api/toyo-weather?date=${dateStr}`)).json();
  ...
  async function load(dateStr) {
    ...
    try {
      ...
      const siteQ = `&site=${encodeURIComponent(site)}`;
      const dP = fetch(`/api/toyo-occupancy?date=${dateStr}&range=${mode}${granQ}${siteQ}${bust}`, { cache: "no-store" }).then((r) => r.json());
      const wxP = mode === "week" ? fetch(`/api/toyo-weather?date=${dateStr}&range=week${bust}`, { cache: "no-store" }).then((r) => r.json()).catch(() => null) : null;
      const cmpVal = $("cmpDate") ? $("cmpDate").value : "";
      const cP = (mode === "day" && cmpOn && cmpVal) ? fetch(`/api/toyo-occupancy?date=${cmpVal}&range=day${siteQ}${bust}`, { cache: "no-store" }).then((r) => r.json()).catch(() => null) : null;
  ...
  const siteSeg = $("siteSeg");
  siteSeg.addEventListener("click", (e) => {
    const b = e.target.closest("button");
    if (!b || b.dataset.site === site) return;
    site = b.dataset.site;
    Array.from(siteSeg.children).forEach((x) => x.classList.toggle("on", x === b));
    load(pick.value);
  });
